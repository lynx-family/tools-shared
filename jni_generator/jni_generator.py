...
class CalledByNative(object):
    ...
    def __init__(self, **kwargs):
        ...
        self.env_call = GetEnvCall(self.is_constructor, self.static, self.return_type)
...
class JniParams(object):
    ...
    def AddAdditionalImport(class_name):
        ...
        if "." in raw_class_name:
            raise SyntaxError(
                "%s cannot be used in @JNIAdditionalImport. "
        ...
        if new_import in JniParams._imports:
            raise SyntaxError(
                "Do not use JNIAdditionalImport on an already "
    ...
    def ExtractImportsAndInnerClasses(contents):
        if not JniParams._package:
            raise RuntimeError(
                "SetFullyQualifiedClass must be called before "
        ...
        for match in re.finditer(re_inner, contents):
            ...
            if not JniParams._fully_qualified_class.endswith(inner):
                JniParams._inner_classes += [
                    JniParams._fully_qualified_class + "$" + inner
                ]
        ...
        if "." in param:
            ...
            for qualified_name in JniParams._imports:
                if qualified_name.endswith("/" + outer):
                    return (
                        prefix
                        + JniParams.RemapClassName(qualified_name)
                        + "$"
                        + inner
                        + ";"
                    )
            raise SyntaxError(
                "Inner class (%s) can not be "
                "used directly by JNI. Please import the outer "
                "class, probably:\n"
                "import %s.%s;"
                % (param, JniParams._package.replace("/", "."), outer.replace("/", "."))
            )
    ...
    def _CheckImplicitImports(param):
        ...
        if not JniParams._implicit_imports:
            ...
            with open(
                os.path.join(os.path.dirname(sys.argv[0]), "android_jar.classes"), "r"
            ) as f:
        ...
        for implicit_import in JniParams._implicit_imports:
            ...
            if implicit_import.endswith("." + param):
                raise SyntaxError(
                    "Ambiguous class (%s) can not be used directly "
    ...
    def SplitParams(params):
        ...
        for sub_string in param_list:
            bracket_num_dev = (
                bracket_num_dev + sub_string.count("<") - sub_string.count(">")
            )
...
def MangleCalledByNatives(called_by_natives):
    ...
    method_counts = collections.defaultdict(lambda: collections.defaultdict(lambda: 0))
    ...
    for called_by_native in called_by_natives:
        ...
        if method_counts[java_class_name][method_name] > 1:
            method_id_var_name = GetMangledMethodName(
                method_name, called_by_native.params, called_by_native.return_type
            )
...
def ExtractCalledByNatives(contents):
    ...
    for line1, line2 in zip(unmatched_lines, unmatched_lines[1:]):
        if "@CalledByNative" in line1:
            raise ParseError(
                "could not parse @CalledByNative method signature", line1, line2
            )
...
class JNIFromJavaP(object):
    ...
    def __init__(self, contents, options):
        ...
        for line in contents:
            class_name = re.match(
                r".*?(public).*?(class|interface) (?P<class_name>\S+?)( |\Z)", line
            )
            if class_name:
        ...
        re_method = re.compile(
            r"(?P<prefix>.*?)(?P<return_type>\S+?) (?P<name>\w+?)"
            r"\((?P<params>.*?)\)"
        )
...
jmethodID g_${JAVA_CLASS}_${METHOD_ID_VAR_NAME} = NULL;"""
    ...
    def SubstituteNativeMethods(self, template):
        ...
        for clazz in all_classes:
            ...
            if kmethods:
                values = {"JAVA_CLASS": clazz, "KMETHODS": kmethods}
                ret += [template.substitute(values)]
        if not ret:
            return ""
        return "\n" + "\n".join(ret)
...
"""
        ...
        values = {
            "FULLY_QUALIFIED_CLASS": java_name,
    ...
    def GetParamsInDeclaration(self, native):
        ...
        """
        return ",\n    ".join(
            self.GetJNIFirstParam(native)
            + [
                JavaDataTypeToC(param.datatype) + " " + param.name
    ...
    def GetStubName(self, native):
        ...
        if self.options.native_exports:
            ...
            values = {"NAME": native.name, "JAVA_NAME": java_name}
            return template.substitute(values)
        else:
            return native.name
...
"""
    ...
    def GetArgument(self, param):
        return "int(" + param.name + ")" if param.datatype == "int" else param.name
    ...
    def GetCalledByNativeValues(self, called_by_native):
        ...
        if return_type != "void":
            ...
            if re.match(RE_SCOPED_JNI_RETURN_TYPES, return_type):
                return_type = (
                    "lynx::base::android::ScopedLocalJavaRef<" + return_type + ">"
                )
...
${FUNCTION_HEADER}
  ...
  ${RETURN_CLAUSE}
        ...
        values = self.GetCalledByNativeValues(called_by_native)
        values["FUNCTION_SIGNATURE"] = function_signature_template.substitute(values)
        if called_by_native.system_class:
            values["FUNCTION_HEADER"] = function_header_with_unused_template.substitute(
                values
            )
        else:
    ...
    def GetKMethodArrayEntry(self, native):
        template = Template(
            '    { "native${NAME}", ${JNI_SIGNATURE}, '
            + "reinterpret_cast<void*>(${STUB_NAME}) },"
        )
        values = {
            "NAME": native.name,
            "JNI_SIGNATURE": JniParams.Signature(
                native.params, native.return_type, True
            ),
            "STUB_NAME": self.GetStubName(native),
        }
        return template.substitute(values)
...
"""
        ...
        if called_by_native.signature:
            ...
            signature = JniParams.Signature(
                called_by_native.params, jni_return_type, True
            )
        values = {
            "JAVA_CLASS": called_by_native.java_class_name or self.class_name,
            "JNI_NAME": jni_name,
    ...
    for line in output.splitlines():
        ...
        if len(line) < 80 or line.lstrip()[:1] == "#":
            ...
            wrapper = textwrap.TextWrapper(
                width=80, subsequent_indent=subsequent_indent, break_long_words=False
            )
...
def GenerateJNIHeader(input_file, output_file, options):
    ...
    try:
        if os.path.splitext(input_file)[1] == ".class":
            ...
            jni_from_java_source = JNIFromJavaSource.CreateFromFile(input_file, options)
...
def parse_args(args):
    ...
    parser.add_argument(
        "--use-base-jni-header",
        dest="use_base_jni_utils_header",
        action="store_true",
        default=False,
    )
